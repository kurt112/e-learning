...
import {Admin_Delete} from "../../../utils/Specify"
...
const reducer = (state = init_state, action) => {
    ...
    switch (action.type) {
        ...
        case globalActionDialog.ADMIN_DIALOG_REGISTER(Admin_Delete):
            return newState.initRegister(state)
        case globalActionDialog.ADMIN_DIALOG_REGISTER_FAIL(Admin_Delete):
            return newState.failRegister(state, action)
        case globalActionDialog.ADMIN_DIALOG_REGISTER_SUCCESS(Admin_Delete):
            return newState.successRegister(state)
        case globalActionDialog.ADMIN_DIALOG_REGISTER_MESSAGE_CLOSE(Admin_Delete):
            return newState.handleClose(state, action)
        case globalActionDialog.ADMIN_DIALOG_ID_CHANGE(Admin_Delete):
            return newState.changeId(state,action.value)

        // error Handler
        case SET_ERROR_ID_EMPTY(Admin_Delete):
