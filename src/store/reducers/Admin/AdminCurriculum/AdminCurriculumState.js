...
const reducer = (state = currentState, action) =>{
    switch(action.type){
        case actions.ADMIN_TABLE_SETTINGS_INIT(Curriculum): return newState.apiSettingsInit(state,action)
        case actions.ADMIN_TABLE_INIT(Curriculum): return newState.initData(state)
        case actions.ADMIN_TABLE_SUCCESS(Curriculum): return newState.successData(state, action,transforms)
        case actions.ADMIN_TABLE_FAIL(Curriculum): return newState.failData(state)
        case actions.ADMIN_TABLE_NEXT_PAGE(Curriculum): return newState.nextData(state,action)
        case actions.ADMIN_TABLE_SEARCH_DATA_CHANGE(Curriculum): return newState.searchChange(state,action)

        // for opening and closing dialog
        case actions.DIALOG_OPEN(Curriculum_Create): return newState.openDialog(state)
        case actions.DIALOG_CLOSE(Curriculum_Create): return newState.closeDialog(state)
