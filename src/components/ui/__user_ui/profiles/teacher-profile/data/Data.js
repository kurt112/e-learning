...
const Data = ({
    ...
    useEffect(() => {
        ...
        setBirthDate(teacher.user.birthdate)
        setEmail(teacher.user.email)
        setGender(teacher.user.gender)
        setMiddleName(teacher.user.middleName)
        setCurrentPassword('')
        setNewPassword('')
        setReTypePassword('')
    }, [update])
    ...
    const saveProfileClick = async () => {
        ...
        const match = await compareBCrypt(teacher.user.password, currentPassword)
        ...
        if (checkStringEmpty(email)) {
            ...
            error = true
        } else if (email !== teacher.user.email) {
        ...
        const params = new URLSearchParams();
        const password = newPassword === '' ? teacher.user.password : await PasswordEncrypt(newPassword)
